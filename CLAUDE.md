...
- v0.7.3 — CV enhancements: (1) Japanese-only CV format configurable per LPK by superadmin (cv_lang_mode + cv_lang_ja_lpk_ids GlobalSettings; all field labels/values in Japanese, 年月日 dates, no text-transform:uppercase; exceptions: address, birthplace, school name, company name, major); (2) Remove Hobi and Motivasi fields from CV; redistribute space to Skill (40→60px, 200→300 chars) and Promosi Diri (60→100px, 300→400 chars); both CandidateCV and batch PDF template updated; Resume column added to Dalam Seleksi table (shown when recruiter resume enabled)
- v0.7.4 — Education history redesign: 4-column form (Periode via month/year dropdowns, Nama Sekolah, Status enum Lulus/Drop Out/Masih Belajar, Jurusan); end date supports "Sekarang (現在)" → null; DB migration 000038 adds status column; CV education table updated to 4 columns with Japanese status labels (卒業/中退/在学中); date format always 年月 in CV
- v0.7.5 — CV/form standardization: (1) Education status merged into 学校名 cell as sub-line (no separate column/header); (2) All dropdown option text uppercased in education form (month names, Sekarang, status values); (3) Career date standardized to 年月 format — migration 000039 adds endDate to candidate_careers; career form replaces freetext Periode + date input with month/year dropdowns (same pattern as education); CV uses formatPeriodJa(startDate, endDate) with period-text fallback for existing data
- v0.7.6 — CV polish: uniform 25/40/35% column layout across all three CV tables (Pendidikan/Pekerjaan/Sertifikasi); 在学状況 shown inline right-aligned in 学校名 cell; base font size bumped 12px→13px; Superadmin Data Entry Settings UX overhaul: auto-save on every control change (8 save buttons removed), Toggle switches replace Active/Inactive radio pairs, CV Font+Layout+Language grouped into one card, Translation merged into single section, Resume sub-options dim when disabled, hardcoded i18n strings fixed
- v0.7.7 — Superadmin Data Entry Settings rebuilt as 6-tab navigation (Profil/CV/Resume/Foto/Terjemahan/Sistem); fix tab labels showing raw i18n keys (locale file changes from prior session were never committed)
