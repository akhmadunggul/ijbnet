...
> Then add an entry to [CHANGELOG.md](./CHANGELOG.md) and update the `Current:` line below.
...
- v0.9.0 — Admin batch import from Excel: POST /api/admin/candidates/import (.xlsx upsert, downloadable template, AdminImportPage with drag-and-drop)
- v0.8.10 — Fix batch CV logo cut off: replace overflow:hidden float clearfix with explicit clear:both div in all three CV HTML templates (v1/v2/v3)
