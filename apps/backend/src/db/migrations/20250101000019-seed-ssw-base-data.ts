...
// All base rows that were previously only in the seeder (not in any migration).
// Migration 000018 already handles the 7 new SSW1 sectors and 9 new SSW2 sectors.
// This migration ensures the original 63 seeder rows are present regardless of seeder state.
const BASE_ROWS: Row[] = [
  // ── SSW1 : 1. 介護
...
module.exports = {
  async up(queryInterface: QueryInterface): Promise<void> {
    ...
    const toInsert = BASE_ROWS.filter((r) => !existingFieldIds.has(r.fieldId));
    if (toInsert.length === 0) {
      console.log('All base SSW sector fields already exist — skipping.');
      return;
    }
    ...
    console.log(`✓ ${toInsert.length} base SSW sector/field rows inserted.`);
  },

  async down(queryInterface: QueryInterface): Promise<void> {
    const fieldIds = BASE_ROWS.map((r) => r.fieldId);
