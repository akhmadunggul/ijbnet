...
try:
    ...
    if bg_color:
        ...
        result = pipe(pil_img, return_mask=True)

        # transformers may return PIL Image directly or a list-of-dicts
        ...
        alpha = cv2.resize(alpha, (img_cv.shape[1], img_cv.shape[0]),
                           interpolation=cv2.INTER_CUBIC)
        # Normalize to 0-1 regardless of whether mask was 0-255 or 0-1
        if alpha.max() > 1.0:
            alpha = alpha / 255.0
