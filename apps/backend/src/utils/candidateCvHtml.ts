...
export function buildCandidateCvHtml(
  ...
  // ── Inline styles (mirrors CandidateCV.tsx S object) ─────────────────────────
  ...
  const careerRowsHtml = careerRows.map((row) =>
    row
      ? `<tr class="cv-row-md"><td style="${TD}height:40px;">${he(v(row['period']))}</td><td style="${TD}">${he(v(row['companyName']))}</td><td style="${TD}">${he(v(row['divisionJa'] as unknown) || v(row['skillGroupJa'] as unknown) || v(row['division'] as unknown) || v(row['skillGroup'] as unknown))}</td></tr>`
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .cv-row-sm { height: 25px; }
    .cv-row-md { height: 40px; }
    .cv-row-lg { height: 50px; }
  </style>
</head>
<body>
<div style="width:800px;margin:0 auto;border:2px solid #000;padding:20px;font-size:12px;color:#000;box-sizing:border-box;text-transform:uppercase;">
